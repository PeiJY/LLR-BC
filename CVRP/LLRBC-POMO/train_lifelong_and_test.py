...
from CVRPTrainer import CVRPTrainer_LLRBC as Trainer_LLRBC
from CVRPTrainer import CVRPTrainer_EWC as Trainer_EWC
from CVRPTrainer import CVRPTrainer_LiBOG as Trainer_LiBOG
...
def main(task_order=['U','E','GM','C','Ring','Grid'],llmethod = 'FT'):
    print('LL method: ',llmethod)
    ...
    if llmethod == 'FT':
        ...
        print('creat trainer: Trainer_LLRBC')
...
def _set_debug_mode():
    ...
    # trainer_params["batch_sizes"] = {'U':64,'Ring':64,'E':32,'Grid':32,'GM':16,'C':16},
...
if __name__ == "__main__":
    ...
    # --- for experience replay and behaviour clone --- 
    parser.add_argument('--consolidation_loss_weight',type=float,default=0.1,help='weight of clone loss')
    parser.add_argument('--buffer_size',type=int,default=40000,help='replay buffer size')
    parser.add_argument('--clone_sample_size',type=int,default=32,help='sample size from the buffer')
    ...
    parser.add_argument('--RKLDw',type=float,default=0.0,help='the weight of RKLD in clone loss, 0.0 for use KLD only, 1.0 for use RKLD only')
    ...
    print('args.consolidation_loss_weight',args.consolidation_loss_weight)
