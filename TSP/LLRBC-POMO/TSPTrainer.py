...
class TSPTrainer_multitask:
    ...
    def change_env(self,env_params,overwrite_self=True,change_trianer_params=True,print_log = True,re_init_optimizer=False):
        if overwrite_self:
            self.env_params = env_params
            self.env = Env(**self.env_params)
        else:
...
class TSPTrainer_EWC:
    ...
    def _train_one_batch(self, batch_size):
        ...
        # print('RL loss: {}. EWC loss: {}. EWC loss times lambda: {}'.format(loss_mean,ewc_loss,self.ewc_lambda * ewc_loss))
...
class TSPTrainer_LiBOG:
    ...
    def _train_one_batch(self, batch_size):
        ...
        self.env.load_problems(batch_size)
        reset_state, _, _ = self.env.reset()
        reset_state_copy = copy.deepcopy(reset_state)
        # self.model.pre_forward(reset_state)
        ...
        # ---- for ITC -----
        probs_list = torch.zeros(size=(batch_size, self.env.pomo_size, self.env.problem_size, 0))
        bsf_probs_list = torch.zeros(size=(batch_size, self.env.pomo_size, self.env.problem_size, 0))
        # ------------------
        ...
        if not self.bsf_model is None:
            self.bsf_model.pre_forward(reset_state_copy)
        ...
        while not done:
            ...
            # ---- for ITC -----
            if not self.bsf_model is None:
                _,_,bsf_probs = self.bsf_model(state_copy)
                # print('probs shape, ',probs.shape)
                # print('bsf_probs shape, ',bsf_probs.shape)
                # print('probs_list shape, ',probs_list.shape)
                # print('bsf_probs_list shape, ',bsf_probs_list.shape)
                bsf_probs_list = torch.cat((bsf_probs_list, bsf_probs[:,:,:self.env.problem_size].unsqueeze(-1)), dim=3)
                probs_list = torch.cat((probs_list, probs[:,:,:self.env.problem_size].unsqueeze(-1)), dim=3)
            # ------------------
            state, reward, done = self.env.step(selected)
    ...
    def compute_kl_divergence(self,P_probs,Q_probs):
        ...
        # print(' -- after flatten, ',P_probs.shape,Q_probs.shape, kl_loss)
        return kl_loss
...
import time
def behaviour_consolidation_loss(model, sampled_experiences, temperature=1.0, reverse_KLD_clone=False, device='cuda',confidence_weight_KLD=False,confidence_method='var',confidence_scale=1.0,RKLD_weight=0.0):
    # t0 = time.time()
    kl_list = []
    # confidence_list = []
    none_count = 0
    for exp in sampled_experiences:
        ...
        for attr in vars(state).keys():
            ...
            if v is not None and torch.is_tensor(v):
                if attr in ['BATCH_IDX', 'POMO_IDX', 'current_node']:
                    v = v.to(device).long()
                else:
                    v = v.to(device).float()
        ...
        _,_,current_logits = model(state,use_state_q1=True)
        ...
        if confidence_weight_KLD:
            ...
            weighted_mean = (weights * kl_flat).sum() / weights.sum()
...
class TSPTrainer_LLRBC:
    def __init__(self,
        ...
        self.reverse_KLD_clone = self.trainer_params['reverse_KLD_clone']
    ...
    def _train_one_epoch(self, epoch):
        ...
        consolidation_loss_AM = AverageMeter()
        ...
        while episode < train_num_episode:
            ...
            avg_score, avg_loss, (RL_loss_value,consolidation_loss_value)  = self._train_one_batch(batch_size,do_experience_collect)
            score_AM.update(avg_score, batch_size)
            loss_AM.update(avg_loss, batch_size)
            RL_loss_AM.update(RL_loss_value, batch_size)
            consolidation_loss_AM.update(consolidation_loss_value, batch_size)
            episode += batch_size

        # Log Once, for each epoch
        loss_terms_info = ''
        if self.consolidation_loss_weight > 0:
            loss_terms_info = ', RL loss: {:.4f} (weight {}), Clone loss: {:.4f} (weight {}), buffer added {}'.format(RL_loss_AM.avg,1-self.consolidation_loss_weight,consolidation_loss_AM.avg,self.consolidation_loss_weight,self.updating_experience_buffer.add_num)
    ...
    def _train_one_batch(self, batch_size, do_experience_collect):
        ...
        self.env.load_problems(batch_size)
        reset_state, _, _ = self.env.reset()
        reset_statecopy = copy.deepcopy(reset_state)
        self.model.pre_forward(reset_state)
        ...
        while not done:
            ...
            # shape: (batch, pomo)
            # ---- add to buffer ----
            if do_experience_collect and step_couter >= 1 and step_couter < self.env.problem_size-1: # the first and last 1 steps are not included, step 1 depot,
                self.updating_experience_buffer.add(state=state_copy,reset_state=reset_statecopy,actions=selected,probs=probs,task=self.env.distribution)
            # -----------------------
            state, reward, done = self.env.step(selected)
            state_copy = copy.deepcopy(state)
            prob_list = torch.cat((prob_list, prob[:, :, None]), dim=2)
        ...
        if self.consolidation_loss_weight > 0 and len(self.old_experience_buffer.buffer) > 0:
            ...
            consolidation_loss = behaviour_consolidation_loss(self.model,sampled_experiences,temperature=self.temperature,reverse_KLD_clone=self.reverse_KLD_clone,
            ...
            consolidation_loss = consolidation_loss * self.consolidation_loss_weight
            consolidation_loss.backward()
            consolidation_loss_value = consolidation_loss.item()
        else:
            loss_mean.backward()
            consolidation_loss_value = 0.0
        ...
        # loss_mean.backward()
        self.optimizer.step()
        return score_mean.item(), consolidation_loss_value+RL_loss_value, (RL_loss_value,consolidation_loss_value)
