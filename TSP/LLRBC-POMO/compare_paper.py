...
else: output_fodler += 'figures_rebuttal_main/'
...
for order_index in range(len(orders)): #distribution_order in orders:
# for order_index in [4]: #distribution_order in orders:
    ...
    for runname in runnames:
        # print(runname,runnames[runname])
        if os.path.exists(dfolder + runname+ '/quick_test_performance.json'):
            ...
            try:
                for distribution in distribution_order:
                    all_objs = [v[score_type] for dic in data.values() for k,v in dic.items() if k == distribution]
...
for order_index in orders: 
    ...
    if remove_unseen:
        tempstr+='_nounseen'
    plt.savefig(folder+str(K)+'_compare'+tempstr+f'_O{order_index}_TSP.pdf')
        # try:
...
with open(output_fodler+"/"+str(K)+"_overall_order_summary.csv", 'w') as file:
    ...
    # writer.writerow(["Name", "AvgRank", "AvgNorm", "AvgFinalForgetRank", "AvgFinalForget", "AvgMaxForgetRank", "AvgMaxForget"])
    writer.writerow(["Name", "AvgNorm",  "AvgFinalForget", "AvgMaxForget", "AvgPlasticity", "AvgGeneralization"])
    ...
    # names = sorted(for_rank.keys(), key=lambda x: sum(for_rank[x])/len(for_rank[x]))
    ...
    for name in names:
        ...
        writer.writerow([
            name,
            # "{:.1f} ({:.1f})".format(sum(avgrank)/len(avgrank),np.std(avgrank, ddof=0)),
            "{:.1f} ({:.1f})".format(sum(avgnorm)/len(avgnorm)* 1000, np.std(avgnorm, ddof=0)*1000),
            # "{:.1f} ({:.1f})".format(sum(rank_avg_forgetting)/len(rank_avg_forgetting), np.std(rank_avg_forgetting, ddof=0)),
            "{:.1f} ({:.1f})".format(sum(avg_avg_forgetting)/len(avg_avg_forgetting)* 1000, np.std(avg_avg_forgetting, ddof=0)*1000),
            # "{:.1f} ({:.1f})".format(sum(rank_max_forgetting)/len(rank_max_forgetting), np.std(rank_max_forgetting, ddof=0)),
            "{:.1f} ({:.1f})".format(sum(avg_max_forgetting)/len(avg_max_forgetting)* 1000, np.std(avg_max_forgetting, ddof=0)*1000),
            # "{:.1f} ({:.1f})".format(sum(rank_plasticity)/len(rank_plasticity), np.std(rank_plasticity, ddof=0)),
            "{:.1f} ({:.1f})".format(sum(avg_plasticity)/len(avg_plasticity)* 1000, np.std(avg_plasticity, ddof=0)*1000),
            # "{:.1f} ({:.1f})".format(sum(rank_generalization)/len(rank_generalization), np.std(rank_generalization, ddof=0)),
            "{:.1f} ({:.1f})".format(sum(avg_generalization)/len(avg_generalization)* 1000, np.std(avg_generalization, ddof=0)*1000),
        ])
        # print(f"{sum(avgrank)/len(avgrank):.1f}\t{sum(avgnorm)/len(avgnorm):.4f}\t{sum(avg_avg_forgetting)/len(avg_avg_forgetting):.4f}\t\t{sum(avg_max_forgetting)/len(avg_max_forgetting):.4f}\t\t{name}\t{len(avgrank)}")
