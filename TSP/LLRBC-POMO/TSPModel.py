...
class TSPModel(nn.Module):
    ...
    def forward(self, state, use_state_q1=False):
        batch_size = state.BATCH_IDX.size(0)
        pomo_size = state.BATCH_IDX.size(1)
        probs = torch.full(
            size=(batch_size, pomo_size, pomo_size + 1),
            fill_value=1.0 / (pomo_size + 1)
        )
        ...
        if state.current_node is None:
            ...
            state.q1 = self.decoder.q_first.detach()

        else:
            if use_state_q1:
                self.decoder.q_first  = state.q1
            ...
            if self.training or self.model_params['eval_type'] == 'softmax':
                while True:
                    ...
                    probs_2d = probs.reshape(batch_size * pomo_size, -1)

                    row_sums = probs_2d.sum(dim=1)
                    invalid_rows = (row_sums <= 0) | torch.isnan(row_sums) | torch.isinf(row_sums)
...
class TSP_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, ninf_mask):
        ...
        all_inf_mask = (mask == float('-inf')).all(dim=-1)
